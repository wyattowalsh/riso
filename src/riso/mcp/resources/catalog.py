...
def register_catalog_resources(mcp: FastMCP) -> None:
    ...
    def get_dependencies_map() -> str:
        ...
        lines = [
            ...
            "",
            "## Python API (`api_module=enabled`, `api_languages` includes `python`)",
            "- `fastapi>=0.115.0` - Web framework",
            "- `uvicorn>=0.32.0` - ASGI server",
            "- `pydantic>=2.0` - Data validation",
            "- `httpx>=0.27.0` - HTTP client",
            "",
            "## Node API (`api_module=enabled`, `api_languages` includes `node`)",
            ...
            "- Requires: `api_module=enabled` and `api_languages` includes `python`",
            "",
            ...
            "",
            "### Sphinx Shibuya (`docs_module=enabled`, `docs_framework=sphinx-shibuya`)",
