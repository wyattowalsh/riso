...
async def compute_diff(
    ...
    with tempfile.TemporaryDirectory(prefix="riso_diff_") as tmpdir:
        ...
        try:
            ...
            if operation == "copy":
                # For copy operations, generate fresh
                # Use run_copy directly to avoid force parameter issues
                ...
                run_copy(
                    ...
                    str(temp_dest),
                    data=answers,
                    unsafe=True,  # Trust the Riso template
                    defaults=True,  # Use defaults for missing answers
                    overwrite=True,  # Allow overwriting (we're in temp dir)
                    skip_tasks=True,  # Skip post-gen tasks for diff
                )
            elif operation == "update":
                # For updates, we need to copy current state first, then update
            ...
            elif operation == "recopy":
                # For recopy, copy current state first, then recopy
                if destination.exists():
                    shutil.copytree(
                        destination,
                        temp_dest,
