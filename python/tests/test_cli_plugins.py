...
from changelog_python.cli.__main__ import app
from changelog_python.cli.core import PluginError, PluginManager
...
class TestPluginDiscovery:
    ...
    def test_plugin_discovery_handles_no_plugins(self, plugin_manager: PluginManager) -> None:
        ...
        with patch(
            "changelog_python.cli.core.plugin_manager.entry_points",
            return_value=[],
        ):
            plugin_manager.discover()
            plugins = plugin_manager.list_plugins()
...
class TestPluginErrorIsolation:
    ...
    def test_failed_plugin_doesnt_crash_discovery(self, plugin_manager: PluginManager) -> None:
        ...
        with patch(
            "changelog_python.cli.core.plugin_manager.entry_points",
            return_value=[mock_ep_good, mock_ep_bad],
        ):
            plugin_manager.discover()

            plugins = plugin_manager.list_plugins()
            assert any(p["name"] == "good_plugin" for p in plugins)
            assert any(p["name"] == "bad_plugin" for p in plugins)

            plugin_manager.load_plugin("good_plugin")
            with pytest.raises(PluginError):
                plugin_manager.load_plugin("bad_plugin")
...
class TestPluginCommands:
    ...
    def test_plugin_list_shows_no_plugins_message(self, cli_runner: CliRunner) -> None:
        ...
        with patch(
            "changelog_python.cli.commands.plugin.plugin_manager.discover",
        ) as mock_discover:
            mock_discover.return_value = None
            with patch(
                "changelog_python.cli.commands.plugin.plugin_manager.list_plugins",
                return_value=[],
            ):
                result = cli_runner.invoke(app, ["plugin", "list"])
