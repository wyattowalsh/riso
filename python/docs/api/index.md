...
The API reference is generated via `sphinx.ext.autodoc` and `sphinx.ext.autosummary`.
...
`src/` and optional workspace packages when present.
...
- Cross-link functions and CLI entrypoints using the default `:py:func:` role for
