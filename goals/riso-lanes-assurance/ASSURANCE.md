...
| 13  | Full `render_matrix.py` completes; never hand-edit renders                               | **residual**         | Owner **PLATFORM** · official W5 run **exited** (`evidence/W5-PL-T06-render_matrix.log`; JSON 2026-08-18T11:48:24Z) with **7 dest fails**. Not green. Individual `render-samples.sh` re-smokes only — no second `render_matrix.py`. Residual `residuals/PLATFORM.md` R1. Do not hand-edit `samples/*/render/`. |
