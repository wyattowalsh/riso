...
- Date: 2026-08-18
- Compared: `goals/riso-v2-release-ready/evidence/W5-R1-CLI.md` (and noted `W5-R2-CLI.md`)
...
Live-tree only. Did **not** treat `ASSURANCE.md`, `W2-CLI-JOIN.md`, `W5-CLOSE-CLI.md`, or residuals as truth. `W5-R1-CLI.md` used as a comparison checklist, not as current status.
...
- `src/riso/template/**` (`__init__.py` `_enforce_generation_gates`, `hooks_runner.py`, `_copier_worker.py`)
...
| W5-R1 item                                   | W6-R03 live                             |
| -------------------------------------------- | --------------------------------------- |
| P0 empty (apply-then-reject + migrate wired) | **still true**                          |
| `CLI-P1-export-list-data`                    | **not still real** — see P1 disposition |
| `W5-R2-CLI.md` said P1 fixed                 | **agrees** on independent re-read       |
...
`--skip-post-gen` remains in `_GLOBAL_FLAGS` (`app.py` L485).
...
### Disposition of W5-R1 `CLI-P1-export-list-data`
...
| **W6-R03 live**   | **fixed / not still real.** `format_data_assignment` (`src/riso/cli/commands/export.py` L20–29) YAML-dumps `list`/`dict` with `default_flow_style=True`. `_coerce_value` (`src/riso/cli/helpers.py` L49–68) `yaml.safe_load`s and returns lists/dicts. `test_export_cli_wrap_list_round_trips_through_parse_and_validate` (`tests/unit/test_cli/test_export.py` L101–128) splits the exported `riso_command`, parses `--data`, asserts `api_languages == ["python"]`, then `resolve_answers` + `validate_and_raise`. |
...
| Class                                    | Count                        |
| ---------------------------------------- | ---------------------------- |
| This-session writes                      | 1 (`evidence/W6-R03-CLI.md`) |
...
Apply-then-reject remains the single remap contract and is still wired through resolve/validate/copy/export/migrate/update/recopy/diff/generation_gates. `riso migrate` is still registered and writes only after apply-then-reject. W5-R1 P0 stays empty. W5-R1 `CLI-P1-export-list-data` is no longer real: wrap-list dests serialize as YAML and re-parse as lists, with a unit round-trip. No `export list` subcommand exists to mis-type `--data`. **P0 empty. P1 empty.**
