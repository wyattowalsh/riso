# W6-R05 — Review, surface=wizard (second dry pass)
...
- Date: 2026-08-18
- Pass: **second dry pass** after `W6-R04-wizard.md`
- Prior review blobs: untrusted. Re-read live `web/src/**` plus Python remap SSOT. `W6-R04-wizard.md` / `W6-R03-wizard.md` / `W5-R1-wizard.md` / `W5-R2-wizard.md` / `W5-CLOSE-WEB.md` / `W5-AUDIT-wizard.md` were used as maps only, not verdicts.
...
Independent re-read of the live wizard remap twin still matches the Python 8-key SSOT. Public import / share / `updateConfig` / export paths still apply then fail-closed. Built-in presets still emit dest keys only. Store + matrix default `task_runner` is still `just`. No new wizard P0 or P1 versus the live tree (and none versus W6-R04).
...
| -------------------------------------------------- | ------------------------------ |
| 8-key remap + fail-closed                          | **still green**                |
| No old keys in 13 `PRESETS`                        | **still green**                |
| Store / matrix `task_runner=just`                  | **still green**                |
| Persist / custom-preset hydrate uses drop (not P1) | **unchanged**                  |
| Stale dest `lucia` persist residual (not P1)       | **unchanged** (no live writer) |
| Scaffolded `api.generateProject` (not P1)          | **unchanged**                  |
| Hero `--data` demo vs Review `--answers-file`      | **unchanged**                  |
| P0 / P1                                            | **still empty**                |
...
Remap is apply-then-reject. `applyRemovedKeyRemaps` then leftover reject. No dest overwrite if dest is set. Idempotent. No dual-path aliases after remap. `saas_auth: lucia` stays an unmapped leftover fail-close (not a dest alias). OpenSpec extra stays off by default.
...
`web/src/components/presets/presets.tsx` `PRESETS[].config` has no `api_tracks` / `api_language` / `docs_site` / `mcp_language` / `saas_starter_module` / `saas_auth` / `saas_billing` / `include_admin`. SaaS presets (`saas-starter`, `enterprise-saas`) use `saas_auth_provider: 'clerk'` only. No preset sets `task_runner` (merge after `resetConfig()` keeps `just`).

Built-in ids (13): `minimal-python`, `python-api`, `rust-cli-python-api`, `go-api`, `typescript-mcp`, `graphql-api`, `fullstack`, `polyglot-monorepo`, `ml-ai-project`, `saas-starter`, `microservices`, `docs-only`, `enterprise-saas`.

Apply path (`presets/index.tsx`): `resetConfig()` then `updateConfig(preset.config)` — remaps fail-closed; omitted keys keep store defaults.

### 3. `task_runner=just`

Default is `just` from live matrix + hardcoded fallback. UI still offers makefile / both / none as **choices**, not defaults. Built-in preset configs omit `task_runner`; `resetConfig()` restores `just` before merge, so applying a preset does not change the default runner.

## Inspected, not elevated (below P1)

Do not reset the dry-review counter on these:

- **Persist / custom-preset hydrate** uses `dropLeftoverRemovedKeys` (`store.ts` `canonicalizeConfig` + persist `merge`, `configSchemas.ts` `parseCustomPresetsStorage`). Comment: hydration, never throw. YAML import, share URL, and `updateConfig` still fail-closed.
- **Stale localStorage dest `saas_auth_provider=lucia`** would still export if a pre-CLOSE-WEB persist existed. No live writer remains. Dest union and `SaaSConfig` fallback are `clerk` | `authjs`. Matrix dest choices are `clerk` / `authjs` only.
