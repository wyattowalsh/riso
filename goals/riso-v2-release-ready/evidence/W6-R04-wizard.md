...
- Date: 2026-08-18
- Prior review blobs: untrusted. Re-read live `web/src/**` plus Python remap SSOT. `W6-R03-wizard.md` / `W5-R1-wizard.md` / `W5-R2-wizard.md` / `W5-CLOSE-WEB.md` / `W5-AUDIT-wizard.md` were not used as verdicts.
...
`web/src/components/presets/presets.tsx` `PRESETS[].config` has no `api_tracks` / `api_language` / `docs_site` / `mcp_language` / `saas_starter_module` / `saas_auth` / `saas_billing` / `include_admin`. SaaS presets (`saas-starter`, `enterprise-saas`) use `saas_auth_provider: 'clerk'` only.
