# W6-R05 — Review pass, surface=gates (read-only, second dry)
...
- Focus: `validate_jinja_templates.py` dir walk, `check_removed_key_ssot.py`, `scripts/lib/sphinx_smoke.py`, `mise trust` in `render-samples.sh`, `riso-release.yml.jinja` `uv --directory python`
- Repo: `/Users/ww/dev/projects/riso`
- Date: 2026-08-18
- Prior blobs (`W4-R03-gates`, `W5-AUDIT-gates`, `W5-R1-gates`, `W5-R2-gates`, `W5-CLOSE-GATES`, `W6-R03-gates`, `W6-R04-gates`, `W6-WF-GHA-release`, `ASSURANCE.md`, `residuals/GATES.md`): **untrusted**. Every keep/drop re-read in the live tree.
...
Older notes that named extra jinja walkers (`validate_jinja_syntax.py`, `check_jinja_undefined.py`, `check_template_hygiene.py`) do **not** exist under `scripts/ci/` in this tree. Official jinja gate is `scripts/ci/validate_jinja_templates.py`.

## Prior findings — disposition

| id                                      | Prior                                          | This pass                                   |
| --------------------------------------- | ---------------------------------------------- | ------------------------------------------- |
| GATES-P0-sphinx-make-linkcheck          | W5-R1 P0; W5-R2 / W6-R03 / W6-R04 source-fixed | **still fixed in source**                   |
| GATES-P1-mise-trust                     | W5-R1 P1; W5-R2 / W6-R03 / W6-R04 source-fixed | **still fixed in source**                   |
...
| GATES-CLOSED-jinja-dir-walk             | closed W4/W5/W6-R03/W6-R04                     | **closed**                                  |
| GATES-CLOSED-ssot-gate                  | closed W4/W5/W6-R03/W6-R04                     | **closed**                                  |
| GATES-CLOSED-quality-ssot-wiring        | closed W4/W5/W6-R03/W6-R04                     | **closed**                                  |
...
| Maintainer `release.yml` Python cwd        | **n/a (repo-root uv)**      | `.github/workflows/release.yml` is the **maintainer** workflow. Maintainer Python lives at repo root (`uv sync --group …`, `uv run pytest`, `uv build`). No dest `python/` subdirectory. Do not treat missing `--directory python` here as a gates P0.                                                                                                                                                                                                                                                                                                                                                           |
| `just validate-agents` wiring              | **closed (source)**         | `justfile:217-230`: dest-independent `validate_agents_ecosystem.py` + `check_quality_parity.py`, then dest-dependent `--render-enabled` `samples/{default,cli-docs,full-stack}/render` + `--render-disabled samples/ai-tools-off/render`. Local recipe does not render. GHA `quality.yml` L216–229 restores those dests via official `render-samples.sh` first.                                                                                                                                                                                                                                                  |
...
| Stale dest smoke JSON                      | **dest-stale, not source**  | `samples/docs-sphinx/smoke-results.json` timestamp `2026-08-14` still records `["uv","run","make","linkcheck"]` + missing `linkcheck` recipe + untrusted dest `mise.toml`. Live source no longer emits that argv and now `mise trust`s before bootstrap. Re-render via the official writer only.                                                                                                                                                                                                                                                                                                                 |
...
Maintainer `.github/workflows/release.yml` is a different tree (repo-root `uv`). Not a dest `python/` cwd bug.
