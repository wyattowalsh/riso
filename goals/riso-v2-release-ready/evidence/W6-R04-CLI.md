...
- Date: 2026-08-18
- Compared: `goals/riso-v2-release-ready/evidence/W6-R03-CLI.md` (P0 empty; P1 empty). Also noted `W5-R1-CLI.md` / `W5-R2-CLI.md`.
...
Live-tree only. Did **not** treat `ASSURANCE.md`, `W2-CLI-JOIN.md`, `W5-CLOSE-CLI.md`, or residuals as truth. `W6-R03-CLI.md` used as the comparison checklist, not as current status.
...
| **W6-R03 / R04**  | **fixed / not still real.** `format_data_assignment` (`src/riso/cli/commands/export.py` L20–29) YAML-dumps `list`/`dict` with `default_flow_style=True`. `_coerce_value` (`src/riso/cli/helpers.py` L49–68) `yaml.safe_load`s and returns lists/dicts. `test_export_cli_wrap_list_round_trips_through_parse_and_validate` (`tests/unit/test_cli/test_export.py` L101–128) splits the exported `riso_command`, parses `--data`, asserts `api_languages == ["python"]`, then `resolve_answers` + `validate_and_raise`. |
...
| Class                                    | Count                        |
| ---------------------------------------- | ---------------------------- |
| This-session writes                      | 1 (`evidence/W6-R04-CLI.md`) |
