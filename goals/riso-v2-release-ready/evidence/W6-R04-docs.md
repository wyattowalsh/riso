# W6-R04 — Review pass, surface=docs
...
- Prior blobs (`W6-R03-docs.md`, `W6-DOCS-changelog.md`, `W5-R1-docs.md`, `W5-R2-docs.md`) used as pointers only; every keep/drop re-read live
- Status: **P0 empty; P1 empty.** Prior `DOCS-P1-01` is closed on live `CHANGELOG.md`.
...
## W6-R03 disposition (re-read, not inherited)

| id           | W6-R03                      | W6-R04 live                                                                                      |
| ------------ | --------------------------- | ------------------------------------------------------------------------------------------------ |
| (P0)         | empty                       | **still empty** — both required pages exist                                                      |
| `DOCS-P1-01` | CHANGELOG omits dest prompt | **closed** — Unreleased 2.0.0 L58–60 names `api_features` as the Copier dest key + enable recipe |

W6-DOCS-changelog recorded the dest-key sentence insert. This pass re-read `CHANGELOG.md` L9–85; it matches that text. Flags are **not** added to the 8-row remap table.
...
None. All four required surfaces name `api_features` as the Copier dest / enable prompt for GraphQL/WebSocket. W6-R03 `DOCS-P1-01` is **not** still real.
...
| DOCS-C02 | `CHANGELOG.md`                               | Section exists; 8 remaps, operators, dests, contract, no-tag sentence; dest-key + enable recipe now lockstep. |
...
Exclusive write this session: `goals/riso-v2-release-ready/evidence/W6-R04-docs.md`.
