...
| **owner**           | PLATFORM                                                                                                                                       |
| **status**          | dest-stale (default dest restored 2026-08-18)                                                                                                  |
| **command**         | Official dest refresh via `render-samples.sh` / existing `render_matrix.json` only. Never hand-edit `samples/*/render/**`.                     |
| **blocking reason** | Default dest + `AGENTS.md` are back; `just validate-agents` is green. Other dests may still be stale until PLATFORM re-renders those variants. |
