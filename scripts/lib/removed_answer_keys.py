...
to an identical local twin so template hooks that only put ``scripts/`` on
``sys.path`` keep working.

The fallback mirrors ``ANSWER_KEY_REMAPS`` + ``apply_removed_key_remaps``
(apply known remaps, leave unmapped leftovers for reject). Do not overwrite a
destination key that is already set. Drop the old key after a successful apply.
...
_FALLBACK_REMOVED_ANSWER_KEYS: dict[str, str] = {
...
@dataclass(frozen=True)
class _FallbackRemapResult:
    """Outcome of applying known removed-key remaps."""

    answers: dict[str, Any]
    ops: tuple[_FallbackRemapOp, ...]


_FALLBACK_ANSWER_KEY_REMAPS: dict[str, _FallbackRemapOp] = {
    "api_tracks": _FallbackRemapOp(
        old="api_tracks",
        new_keys=("api_module", "api_languages"),
        action="derive",
    ),
    "api_language": _FallbackRemapOp(
        old="api_language",
        new_keys=("api_languages",),
        action="wrap-list",
    ),
    "docs_site": _FallbackRemapOp(
        old="docs_site",
        new_keys=("docs_module", "docs_framework"),
        action="derive",
    ),
    "mcp_language": _FallbackRemapOp(
        old="mcp_language",
        new_keys=("mcp_languages",),
        action="wrap-list",
    ),
    "saas_starter_module": _FallbackRemapOp(
        old="saas_starter_module",
        new_keys=("saas_infra_module",),
        action="rename",
    ),
    "saas_auth": _FallbackRemapOp(
        old="saas_auth",
        new_keys=("saas_auth_module", "saas_auth_provider"),
        action="split",
    ),
    "saas_billing": _FallbackRemapOp(
        old="saas_billing",
        new_keys=("saas_billing_module", "saas_billing_provider"),
        action="split",
    ),
    "include_admin": _FallbackRemapOp(
        old="include_admin",
...
def _fallback_apply_removed_key_remaps(
    answers: Mapping[str, Any],
) -> _FallbackRemapResult:
    """Apply known removed-key remaps. Leave unmapped leftovers for reject."""
    out = dict(answers)
    ops: list[_FallbackRemapOp] = []
    for old, spec in _FALLBACK_ANSWER_KEY_REMAPS.items():
        if old not in out:
            continue
        before = out[old]
    ...
    return _FallbackRemapResult(answers=out, ops=tuple(ops))
...
if TYPE_CHECKING:
    ...
    try:
        ...
        ANSWER_KEY_REMAPS = _FALLBACK_ANSWER_KEY_REMAPS
        REMOVED_ANSWER_KEYS = _FALLBACK_REMOVED_ANSWER_KEYS
        RemapOp = _FallbackRemapOp
        RemapResult = _FallbackRemapResult
        apply_removed_key_remaps = _fallback_apply_removed_key_remaps
