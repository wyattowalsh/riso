...
1. **Cache strategy**: Lock file hash + OS/Python version prefix
1. **Matrix configuration**: Python 3.11-3.13, fail-fast disabled
1. **Retry logic**: 3 attempts with exponential backoff
1. **Parallelization**: Python and Node jobs always parallel
1. **Artifact retention**: 90 days per constitution
1. **Validation**: actionlint in post-generation hook
1. **Quality profiles**: Integrated with feature 003 settings
1. **Module conditionals**: Jinja2 conditionals for module-specific checks
1. **Timeout configuration**: 10 min standard, 20 min strict
...
1. **Foundational**: Implement workflow generation and validation hooks
1. **User Story 1**: Main quality workflow with retry logic and artifact uploads
1. **User Story 2**: Matrix testing across Python versions
1. **User Story 3**: Cache configuration with lock file hashing
1. **User Story 4**: Artifact collection and retention
1. **User Story 5**: Conditional Node.js track integration
1. **Polish**: Documentation updates, smoke tests, governance tracking, free tier optimization
...
1. **Execute tasks** following task dependency order (Setup → Foundational → User Stories → Polish)
1. **Validate each phase** via smoke tests and success criteria checks
1. **Update governance** (AGENTS.md, context files, module success tracking)
1. **Prepare PR** with all artifacts and evidence
