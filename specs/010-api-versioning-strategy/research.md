...
1. **Compile regex patterns once** - Pre-compile URL version patterns at initialization
1. **Use bytecode operations** - Parse headers as bytes before converting to strings
1. **Lazy query parsing** - Only parse query string if header/URL don't contain version
1. **Immutable dataclasses** - Use `frozen=True` for thread safety and optimization
1. **Singleton registry** - Single load at startup, shared across all workers
1. **Benchmark continuously** - Use `pytest-benchmark` to validate \<10ms overhead target

## Next Steps

1. **Phase 1**: Generate data model with entities (VersionMetadata, VersionRegistry, etc.)
1. **Phase 1**: Create API contracts for version discovery endpoint
1. **Phase 1**: Write quickstart guide with integration examples
1. **Phase 2**: Break down implementation into tasks
