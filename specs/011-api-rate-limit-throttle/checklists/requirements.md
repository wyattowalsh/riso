...
1. **Redis Version:** Requires Redis ≥6.0 for ACL support. Older versions lack security features.
1. **Load Balancer Setup:** Assumes X-Forwarded-For header present and trusted. Alternative setups (TCP proxies) need different client identification.
1. **Time Windows:** Specification assumes 60-second windows are sufficiently granular. Sub-second windows may require different algorithms.
...
1. **Proceed to `/speckit.clarify`** to validate requirements with agents/teams
1. **Address any clarifications** before moving to planning phase
1. **Create GitHub issue** (if applicable) linking to this spec
