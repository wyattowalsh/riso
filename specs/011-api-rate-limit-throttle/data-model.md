...
1. **Update**: On each request, refill tokens based on elapsed time, then consume requested amount
1. **Expiration**: TTL set to `window * 2` (e.g., 120s for 60s window) for garbage collection
...
1. **Update**: On subsequent violations, add new timestamps
1. **Cleanup**: `ZREMRANGEBYSCORE` removes violations older than detection window
1. **Expiration**: TTL set to detection_window (e.g., 3600s for 1 hour)
...
1. **Cache**: Store in memory for fast access
1. **Reload**: On SIGHUP signal or config file change, re-parse and update cache
1. **Validation**: Pydantic validates types, ranges (e.g., limit > 0, window ≥ 1)
...
1. **Injection**: Add to response headers (all responses: 200, 429, 5xx)
1. **Client Parsing**: Client reads headers to implement retry logic
...
1. **Client ID + Endpoint → Counter** (N:1): Multiple requests share the same counter
1. **Client ID → Penalty Tracker** (1:1): One penalty tracker per client
1. **Config → Multiple Endpoints** (1:N): One config defines limits for many endpoints
1. **Request → Headers** (1:1): Every response includes rate limit headers
...
1. Background job copies data to new keys
1. Switch writes to new keys after 2× window period (all old keys expired)
1. Remove old key read logic after verification
...
1. Progressive Penalty Tracker (ZSET) - Violation timestamps
1. Configuration Cache (in-memory) - TOML config
1. Rate Limit Headers (HTTP) - Client-facing status
1. Client Identifier (string) - IP or user ID
