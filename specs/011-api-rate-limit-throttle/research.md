...
1. **Redis transactions (MULTI/EXEC)**: Rejected due to complexity and watch/retry logic overhead
1. **Sorted sets (sliding window)**: Rejected as default due to higher memory overhead and complexity; kept as optional algorithm
...
1. **Redis Cluster (6+ nodes)**: Overkill for rate limiting workload; added complexity of sharding not needed unless >100k req/s
1. **5-node Sentinel**: Higher reliability but increased cost; 3-node sufficient for rate limiting use case
...
1. **Trust first IP only**: Rejected; incorrect for multi-proxy setups (gets proxy IP, not client)
1. **Ignore header entirely**: Rejected; breaks rate limiting behind load balancers (all requests appear from LB IP)
...
1. **Linear penalty increase**: Rejected as less effective deterrent than exponential
1. **Permanent bans**: Rejected; out of scope (requires manual intervention and appeals process)
...
1. **Route dependencies**: Rejected; must be added to each route definition (not global)
1. **ASGI middleware**: Considered but Starlette middleware simpler and sufficient
...
1. **Redis Topology**: Sentinel 3-node (1 master + 2 replicas)
1. **IP Extraction**: Rightmost untrusted IP with configurable trust depth
1. **Progressive Penalties**: ZSET-based sliding window with exponential multipliers
1. **Middleware**: Starlette BaseHTTPMiddleware with dependency injection
1. **Metrics**: 5 core metrics with cardinality control
1. **Testing**: 4-tier strategy (unit → integration → load → chaos)
