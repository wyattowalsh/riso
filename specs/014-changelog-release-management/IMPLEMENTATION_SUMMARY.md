...
1. **changelog-monorepo**: Monorepo with independent package versioning
1. **changelog-full-stack**: Python + Node with multi-registry publishing
...
1. **Quickstart Guide** (`specs/014-changelog-release-management/quickstart.md`):
...
1. **Upgrade Guide** (`docs/upgrade-guide.md.jinja`):
...
1. **Manual Hook Installation**: Git hooks require manual installation via script (not automatic during copier generation)
1. **Single-Branch Focus**: Default configuration optimized for main-branch releases (beta/alpha require additional setup)
1. **Registry Dependencies**: Publishing requires pre-configured credentials in GitHub Secrets
...
1. Validate configurations with CI scripts
1. Update branch protection rules to require quality checks
1. Configure GitHub Secrets for registries
1. Test first release in sample project
