...
- Q: When commit validation fails or hooks malfunction, what level of logging and diagnostics should the system provide to help developers and maintainers troubleshoot issues? → A: Structured logging with configurable verbosity levels (normal/verbose/debug modes)
- Q: When the automatic hook installation fails (e.g., due to file system permissions, missing dependencies, or Git configuration issues), how should the system respond to ensure developers can still proceed with their work? → A: Graceful degradation with clear recovery instructions - Display warning with step-by-step fix guide, allow commits to proceed
- Q: For projects with monorepo structure, what are the practical limits for the number of custom commit scopes that the guided authoring tool should support while maintaining usability and performance? → A: Support up to 50 scopes with autocomplete/search for better UX
...
- **Given** a developer needs to uninstall hooks, **When** they delete `.git/hooks/commit-msg`, **Then** validation no longer runs (clean uninstall)
6. **Given** hook installation fails due to permissions, **When** the setup script runs, **Then** it displays a warning with the error reason, manual installation steps, and allows the process to complete without blocking
7. **Given** a developer makes a revert commit, **When** they use message starting with "Revert", **Then** validation is skipped (similar to merge commits)
...
- **What happens when hook installation fails due to permissions or missing dependencies?** The setup script displays a warning with the failure reason and step-by-step recovery instructions (manual hook installation, permission fixes, or dependency installation), then allows the setup process to complete. Developer can proceed with commits, relying on CI validation as fallback.
...
**How does the system handle revert commits generated by Git?** Revert commits typically have format "Revert 'commit message'" which doesn't follow conventional commit format. The validation skips revert commits by checking for "Revert" prefix (similar to merge commit handling).

**How does commit tooling work in CI environments where Git hooks don't exist?** CI validation runs the validation tool directly on commit messages, not via hooks. The workflow checks all commits in a PR against conventional commit rules.
...
- **What happens when a monorepo has more than 50 packages/scopes?** The system enforces a 50-scope limit in configuration. Projects exceeding this should group related packages under common scopes (e.g., "services/auth" → "services") or use alternative tooling designed for very large monorepos (Nx, Turborepo).
...
- **FR-015**: System MUST skip validation for merge commits (starting with "Merge ") and revert commits (starting with "Revert ")
...
- **FR-021**: System MUST provide structured logging with configurable verbosity levels (normal, verbose, debug) for troubleshooting validation failures and hook malfunctions
- **FR-022**: Logging output MUST include timestamp, severity level, validation rule violated, and suggested fix when in verbose or debug mode (see contracts/hook-interface.md for structured format specification)
- **FR-023**: Hook installation script MUST implement graceful degradation when installation fails, displaying warning with step-by-step recovery instructions while allowing setup process to complete
- **FR-024**: Hook installation failure warnings MUST include: failure reason, manual installation steps, CI validation reminder, and documentation link
- **FR-025**: Guided authoring tool MUST support up to 50 custom commit scopes in monorepo configurations
- **FR-026**: When more than 10 scopes are configured, guided authoring tool MUST provide autocomplete/search functionality for scope selection
- **FR-027**: Scope autocomplete MUST support fuzzy matching (partial string matching) to improve discoverability
...
- **Commit Scope**: Optional namespace indicating affected area (e.g., "api", "cli", "docs"). In monorepo, typically matches package name. System supports up to 50 configured scopes with autocomplete/search enabled when count exceeds 10.
...
- **SC-003**: Invalid commit messages are rejected within 1 second with clear error messages (target: <500ms p95 on 2020+ hardware with 2.0+ GHz processor, 8GB+ RAM)
...
- **SC-009**: Tool startup time (hook execution or CLI launch) is under 500ms p95 on standard hardware (2020+ hardware with 2.0+ GHz processor, 8GB+ RAM)
...
- **Logging overhead must not exceed 50ms**: Structured logging in normal mode should have minimal performance impact

### Scalability Constraints

- **Maximum custom scopes**: System supports up to 50 custom commit scopes in configuration
- **Scope selection UX threshold**: When scope count exceeds 10, autocomplete/search interface activates automatically
- **Autocomplete performance**: Fuzzy search must return filtered results within 100ms for up to 50 scopes
- **Configuration file size**: Configuration parsing must handle files up to 10KB without performance degradation
...
- **Logging modes**: System must support three verbosity levels - normal (errors only), verbose (errors + warnings + info), debug (all events including validation checks)
- **Log output**: Normal mode outputs to stderr, verbose/debug modes support optional log file for persistent debugging
- **Log format**: Structured format with timestamp, level, component (hook/CLI/CI), message, and context (commit SHA, rule violated, suggested fix)
- **Log retention**: Local log files (if enabled) managed by user, CI logs retained per existing artifact retention policy (90 days)
