...
1. **Built**: Image created with digest and tags
1. **Scanning**: Trivy/Grype analyzing image layers
1. **Scanned**: Scan results available (may fail if vulnerabilities found)
1. **Publishing**: Image pushing to registry
1. **Published**: Image available at registry URL with tags
...
1. **Linting**: hadolint validates Dockerfile syntax
1. **Building**: Multi-stage build with caching
1. **Scanning**: Trivy/Grype security scan
1. **Publishing** (if main branch): Push to registry with tags
1. **Completed**: Artifacts uploaded, notifications sent
