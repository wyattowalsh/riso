...
1. **Framework consolidation**: Next.js dominates (~70%), with multi-framework support (Next.js + SvelteKit + Nuxt) becoming a differentiator
1. **Auth convergence**: Password + Magic Links + OAuth + MFA are baseline; passkeys gaining but not yet mainstream
1. **Payment standardization**: Stripe + Lemon Squeezy is the universal pairing; MoR support (Paddle, Lemon Squeezy) growing for global SaaS
1. **AI is table stakes**: Every premium starter now includes AI SDK scaffolding; vector DB support expected
...
1. **MCP server scaffolding** - Emerging AI tooling, unaddressed
1. **Copier-based templating** - More flexible than clone-based starters
1. **Modular opt-in features** - Most starters are monolithic
1. **Self-hosting focus** - Premium starters favor Vercel; self-hosting underserved
...
1. **Multi-Tenancy Implementation Gaps** (Severity: MEDIUM)
...
1. **Production Observability** - Logging, metrics, tracing (PostHog, Sentry)
1. **Admin Panels** - User/team/plan management dashboards
1. **Email Infrastructure** - Transactional + marketing workflows
1. **Database Flexibility** - Choice of PostgreSQL/MySQL/MongoDB
...
1. **Python + Node.js** - Only template supporting both ecosystems
1. **MCP tooling** - First to include AI agent scaffolding
1. **Self-hosting first** - Docker support as first-class citizen
1. **Modular architecture** - Truly opt-in features, not monolithic
1. **Active maintenance** - Leverage your template update workflow
