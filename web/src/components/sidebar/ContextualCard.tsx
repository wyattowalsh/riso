...
export function ContextualCard({ currentStep }: ContextualCardProps) {
          ...
          {MODULE_QUICK_TOGGLES.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button
                key={key}
                onClick={() =>
                  updateConfig({ [key]: isEnabled ? "disabled" : "enabled" })
                }
                className={cn(
          ...
          {DOCS_FRAMEWORKS.map((fw) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  updateConfig({
                    docs_framework: fw.id as
                      "fumadocs" | "sphinx-shibuya" | "docusaurus",
                    docs_module: "enabled",
                  })
                }
                className={cn(
        ...
        {hasAnyCost ? (
          <div className="space-y-2 text-xs">
              ...
              <>
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">
                    {config.saas_hosting === "cloudflare"
                      ? "Cloudflare"
                      : "Vercel"}
                  </span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">
                    {config.saas_database === "supabase" ? "Supabase" : "Neon"}{" "}
                    DB
                  </span>
                  <span className="text-gray-900 dark:text-white">
                    Free tier
            ...
            {hasBilling && (
              <div className="flex justify-between">
                <span className="text-gray-500 dark:text-gray-400">
                  {config.saas_billing_provider === "lemonsqueezy"
                    ? "LemonSqueezy"
                    : "Stripe"}
                </span>
                <span className="text-gray-900 dark:text-white">
        ...
        <div className="space-y-2 text-xs">
          {[
            ...
            { label: "Copilot config", done: true },
            {
              ...
              done: config.ai_tools_module === "enabled",
            },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-2">
              ...
              >
                {item.done && (
                  <CheckCircle2 className="h-3 w-3 text-riso-green" />
                )}
              </div>
              <span
                className={
                  item.done ? "text-gray-900 dark:text-white" : "text-gray-400"
                }
              >
                {item.label}
              </span>
  ...
  if (currentStep === 5) {
    ...
    const enabledCount = [
      ...
      config.mcp_module,
      config.docs_module,
      config.codegen_module,
      config.changelog_module,
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-gray-900 dark:text-white">
              {config.project_layout || "single"}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-gray-400">Modules</span>
            <span className="text-riso-federal-blue dark:text-riso-cornflower font-bold">
              {enabledCount}
            </span>
          </div>
        </div>
