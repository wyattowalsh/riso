...
export interface RisoConfig {
  ...
  openspec_extra: "disabled" | "enabled";
  ...
  saas_auth_provider: "clerk" | "authjs";
...
const API_FEATURE_VALUES = new Set([
  "none",
  "graphql",
  "websocket",
  "graphql,websocket",
...
/** Coerce matrix [] / comma lists into the wizard toggle model. */
export function normalizeApiFeatures(
  value: unknown,
): RisoConfig["api_features"] {
  ...
  const hasGraphql = tokens.includes("graphql");
  const hasWebsocket = tokens.includes("websocket");
  if (hasGraphql && hasWebsocket) return "graphql,websocket";
  if (hasGraphql) return "graphql";
  if (hasWebsocket) return "websocket";
  if (typeof value === "string" && API_FEATURE_VALUES.has(value)) {
    return value as RisoConfig["api_features"];
  }
  return "none";
}

export const defaultRisoConfig: Partial<RisoConfig> = {
  // Project basics
  project_name: "",
  ...
  openspec_extra: fromMatrix("openspec_extra", "disabled"),
  ...
  api_features: normalizeApiFeatures(fromMatrix("api_features", "none")),
  ...
  docusaurus_live_codeblock: fromMatrix(
    "docusaurus_live_codeblock",
    "disabled",
  ),
  docusaurus_show_last_update: fromMatrix(
  ...
  docusaurus_autolink_headings: fromMatrix(
  ...
  saas_observability_structured_logging: fromMatrix(
...
export function normalizeCopierWhenDefaults(
  ...
  const next = { ...config };
  next.api_features = normalizeApiFeatures(next.api_features);
...
export const useRisoStore = create<RisoStore>()(
  persist(
    (set, get) => ({
      ...
      resetConfig: () =>
        set({
          config: normalizeCopierWhenDefaults({ ...defaultRisoConfig }),
          currentStep: 0,
        }),
      ...
      loadFromHistory: (id) => {
        ...
        if (history) {
          set({
            config: normalizeCopierWhenDefaults(
              canonicalizeConfig(structuredClone(history.config)),
            ),
          });
        }
      },
